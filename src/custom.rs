...
mod tests {
    ...
    fn make_state(lines: Vec<String>, selected: Vec<bool>, command: &str) -> CustomState {
        CustomState {
            lines,
            selected,
            cursor: 0,
    ...
    fn test_build_commands_append() {
        let state = make_state(
            vec![
                "ssh root@ip1".to_string(),
                "ssh root@ip2".to_string(),
                "ssh root@ip3".to_string(),
            ],
            vec![false, true, true],
            "lsblk",
        );
